...
# Optional Telegram user-ID allowlist for private admin controls. When unset, the
# private bot owner remains compatible with the existing private admin menu.
def read_admin_user_ids() -> set[int]:
...
# Confirmed by the owner through the ID-only admin setup window. Render can
# override or extend this with ADMIN_USER_IDS=123,456 without changing the code.
DEFAULT_ADMIN_USER_ID = 5271474674
ADMIN_USER_IDS = read_admin_user_ids() or {DEFAULT_ADMIN_USER_ID}
...
class Store:
    ...
    def admin_dashboard(self) -> dict[str, Any]:
        """Aggregate non-sensitive dashboard metrics across monitored group chats."""
        with self.connect() as conn:
            groups = list(conn.execute(
                "SELECT chat_id,title,updated_at FROM chats WHERE chat_id < 0 ORDER BY updated_at DESC"
            ).fetchall())
            users = conn.execute("SELECT COUNT(DISTINCT user_id) AS total FROM users").fetchone()
            messages = conn.execute("SELECT COUNT(*) AS total FROM conversation_messages WHERE role='user'").fetchone()
        ...
        return {
            ...
            "announcements": int(announcements["total"] if announcements else 0),
            "alerts": int(warnings["total"] if warnings else 0),
            "users": int(users["total"] if users else 0),
            "messages": int(messages["total"] if messages else 0),
            "ai_replies": int(ai_replies["total"] if ai_replies else 0),
            "news": int(news["total"] if news else 0),
            "music": 0,
            "groups": groups,
        }
...
def private_admin_allowed(user_id: int | None) -> bool:
    """Apply the optional private-chat admin allowlist without breaking legacy setup."""
    return not ADMIN_USER_IDS or (user_id is not None and user_id in ADMIN_USER_IDS)


def admin_control_markup() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🛡 Moderatsiya", callback_data="admin_center:moderation"), InlineKeyboardButton("📊 Statistika", callback_data="admin_center:stats")],
        [InlineKeyboardButton("📣 Majburiy obuna", callback_data="admin_center:subscription"), InlineKeyboardButton("🚫 Filtrlar", callback_data="admin_center:filter")],
        [InlineKeyboardButton("🗓 Rejalashtirilgan post", callback_data="admin_center:schedule")],
        [InlineKeyboardButton("⚙️ Sozlamalar", callback_data="admin_center:settings"), InlineKeyboardButton("📘 Qo‘llanma", callback_data="admin_guide:0")],
        [InlineKeyboardButton("✖️ Yopish", callback_data="admin_center:close")],
    ])


def admin_main_keyboard() -> ReplyKeyboardMarkup:
    """Persistent private admin entry keyboard; Telegram renders these as large chat buttons."""
    return ReplyKeyboardMarkup(
        [["⚙️ Admin boshqaruvi"]],
        resize_keyboard=True,
        is_persistent=True,
        input_field_placeholder="Admin boshqaruvini oching",
    )


def admin_reply_keyboard() -> ReplyKeyboardMarkup:
    """Screenshot-matched two-column Reply Keyboard for authorized admins."""
    return ReplyKeyboardMarkup(
        [
            ["📊 Umumiy ko‘rinish", "👥 Foydalanuvchilar"],
            ["✅ Tasdiqlash", "⚠️ Shikoyatlar"],
            ["💳 To‘lovlar", "🎁 Premium berish"],
            ["📣 Ommaviy xabar", "📅 Tadbirlar"],
            ["⚙️ Sozlamalar"],
            ["🏠 Admin menyu"],
            ["➕ Yangi e’lon"],
            ["🛡 Moderatsiya", "🎯 Viktorina"],
            ["⏱ Avto xabar", "📈 Statistika"],
            ["⏸ Avto xabarni o‘chirish", "▶️ Avto xabarni yoqish"],
        ],
        resize_keyboard=True,
        is_persistent=True,
...
def admin_dashboard_text() -> str:
    metrics = store.admin_dashboard()
    return (
        "⚙️ *Dadasi Admin Control Center*\n\n"
        "Assalomu alaykum, admin. Botning joriy holati:\n"
        "🟢 Webhook: online\n\n"
        "📌 *Asosiy ko‘rsatkichlar*\n"
        f"👥 Faol guruhlar: *{metrics['active_groups']}*\n"
        f"🗂 Jami guruhlar: *{metrics['total_groups']}*\n"
        f"📣 E’lonlar: *{metrics['announcements']}*\n"
        f"⚠️ Alertlar: *{metrics['alerts']}*\n\n"
        "🌐 Javob tili: *O‘zbekcha AI + ko‘p tilli*\n\n"
        "🧠 *Bot statistikasi*\n"
        f"👤 Foydalanuvchilar: *{metrics['users']}*\n"
        f"💬 Jami xabarlar: *{metrics['messages']}*\n"
        f"🤖 AI javoblar: *{metrics['ai_replies']}*\n"
        f"📰 Yangiliklar: *{metrics['news']}*\n"
        f"🎵 Musiqa: *{metrics['music']}*\n\n"
        "🔐 Owner tasdig‘i va e’lon yuborish nazorati faol."
    )


def admin_reference_section(text: str) -> str:
    metrics = store.admin_dashboard()
    if text == "👥 Faol guruhlar":
        if not metrics["groups"]:
            return "👥 *Faol guruhlar*\n\nHozircha bot kuzatayotgan guruhlar yo‘q."
        lines = ["👥 *Faol guruhlar*\n\n"]
        for row in metrics["groups"][:20]:
            title = row["title"] or str(row["chat_id"])
            lines.append(f"🟢 {title} — admin/approved\n")
        return "".join(lines)
    if text == "🗂 Jami guruhlar":
        return f"🗂 *Jami guruhlar*\n\nKuzatilayotgan guruhlar: *{metrics['total_groups']}*"
    if text == "📣 E’lonlar":
        return f"📣 *E’lonlar*\n\nAudit tarixidagi/rejalashtirilgan e’lonlar: *{metrics['announcements']}*\n\nYangi e’lon yuborish uchun `📤 E’lon yuborish` tugmasini tanlang."
    if text == "⚠️ Alertlar":
        return f"⚠️ *Alertlar*\n\nJami ogohlantirishlar: *{metrics['alerts']}*\n\nModeratsiya jurnali uchun /statistika buyrug‘idan foydalaning."
    if text == "🧠 Bot statistikasi":
        return ("🧠 *Bot statistikasi*\n\n"
                ...
                f"📰 Yangiliklar: *{metrics['news']}*\n"
                f"🎵 Musiqa: *{metrics['music']}*")
    if text == "👤 So‘nggi faollik":
        if not metrics["groups"]:
            return "👤 *So‘nggi faollik*\n\nHozircha faollik ma’lumoti yo‘q."
        return "👤 *So‘nggi faollik*\n\n" + "\n".join(f"• {row['title'] or row['chat_id']}" for row in metrics["groups"][:10])
    if text == "📤 E’lon yuborish":
        return "📤 *E’lon yuborish*\n\nE’lon matnini yuborish uchun `/broadcast` buyrug‘i kerak. E’lon faqat admin tasdig‘idan keyin yuboriladi."
    if text == "🌐 Javob va yangilik sozlamalari":
        return "🌐 *Javob va yangilik sozlamalari*\n\nJavob tili: O‘zbekcha AI + foydalanuvchi tilini avtomatik aniqlash\nYangilik manbalari: hozircha sozlanmagan\nTriggerlar: guruhdagi oddiy xabarlar va media\n\nBu bo‘lim faqat admin ko‘rishi uchun ochiq."
    if text == "⚙️ Guruh sozlamalari":
        return "⚙️ *Guruh sozlamalari*\n\n/sozlamalar — joriy holat\n/filtr — spam/reklama filtri\n/majburiy_royxat — majburiy obuna guruhlari"
    if text == "🛡 Xavfsizlik holati":
        return "🛡 *Xavfsizlik holati*\n\n✅ Owner admin allowlist bilan himoyalangan\n✅ Admin buyruqlari Telegram huquqi bilan tekshiriladi\n✅ Spam/reklama filtri mavjud\n✅ E’lonlar tasdiqlashdan keyin yuboriladi"
    if text == "✅ Tasdiqlash":
        return "✅ *Tasdiqlash*\n\nE’lon yuborishdan oldin matn va guruhni tekshiring. Tasdiqlash amali hozircha `/broadcast` oqimiga tayyorlanmoqda."
    return ""


async def admin_reply_button(update: Update, text: str) -> bool:
    """Handle a Reply Keyboard label; return False when the text is not a panel label."""
    if text == "⚙️ Admin boshqaruvi":
        ...
        return True
    if text == "📘 Adminlar uchun qo‘llanma":
        await update.effective_message.reply_text(
            admin_guide_plain_text(ADMIN_GUIDE_PAGES[0]),
            reply_markup=admin_guide_markup(0),
        )
        return True
    reference_section = admin_reference_section(text)
    ...
    action = sections.get(text)
    if action:
        await update.effective_message.reply_text(
            admin_control_section_text(action),
            parse_mode="Markdown",
            reply_markup=admin_reply_keyboard(),
        )
        return True
    notices = {
        "💳 To‘lovlar": "💳 To‘lovlar\n\nTo‘lov holati bo‘yicha sozlamalar hali ulanmagan. Bu tugma xavfsiz ma’lumot oynasi sifatida ishlaydi.",
        "⚠️ Shikoyatlar": "⚠️ Shikoyatlar\n\nShikoyatlarni ko‘rish uchun /statistika buyrug‘idan foydalaning.",
        "🎁 Premium berish": "🎁 Premium berish\n\nPremium berish funksiyasi hali alohida billing xizmatiga ulanmagan.",
        "📣 Ommaviy xabar": "📣 Ommaviy xabar\n\nOmmaviy xabar yuborishdan oldin /yordam yoki /sozlamalar orqali mavjud admin imkoniyatlarini tekshiring.",
        "➕ Yangi e’lon": "➕ Yangi e’lon\n\nE’lon matnini tayyorlang, keyin 📣 Ommaviy xabar yoki /broadcast buyrug‘i orqali yuboring. Yuborishdan oldin tasdiqlash talab qilinadi.",
        "🎯 Viktorina": "🎯 Viktorina\n\nViktorina yaratish funksiyasi keyingi ulanish uchun tayyor. Hozircha /oyin orqali guruhda tezkor o‘yin boshlash mumkin.",
        "⏱ Avto xabar": "⏱ Avto xabar\n\nAvtomatik va rejalashtirilgan xabarlar uchun /rejalashtir buyrug‘idan foydalaning.",
        "⏸ Avto xabarni o‘chirish": "⏸ Avto xabarni o‘chirish\n\nAvto xabarni o‘chirish sozlamasi tanlandi. Faol rejalashtirilgan postlarni /rejalashtir orqali tekshiring.",
        "▶️ Avto xabarni yoqish": "▶️ Avto xabarni yoqish\n\nAvto xabarni yoqish sozlamasi tanlandi. Yangi post uchun /rejalashtir buyrug‘idan foydalaning.",
    }
    if text in notices:
        await update.effective_message.reply_text(notices[text], reply_markup=admin_reply_keyboard())
        return True
    if text == "🏠 Admin menyu":
        await update.effective_message.reply_text(
            "🏠 Admin menyu ochildi. Kerakli bo‘limni tanlang:",
...
def admin_control_section_text(action: str) -> str:
    section = ADMIN_CONTROL_SECTIONS.get(action)
    if section:
        return section
    return "Admin bo‘limi topilmadi."


ADMIN_CONTROL_SECTIONS = {
    "moderation": "🛡 *Moderatsiya boshqaruvi*\n\n"
        "Foydalanuvchi xabariga reply qilib ishlating:\n"
        "/ogohlantir — ogohlantirish\n/jim 10 — 10 daqiqaga jim qilish\n"
        "/jimdanchiqar — jimlikni olish\n/blok — ban\n/blokdanchiqar — unban\n/hayda — guruhdan chiqarish",
    "stats": "📊 *Statistika*\n\n"
        "/statistika — guruh statistikasi, ballar, ogohlantirishlar va oxirgi admin amallari\n"
        "/obuna_statistika — majburiy obuna natijalari\n/obuna_kimlar — o‘tgan foydalanuvchilar ro‘yxati",
    "subscription": "📣 *Majburiy obuna*\n\n"
        "/majburiy_qosh @guruh 24soat — ulash\n/majburiy_royxat — ulangan guruhlarni ko‘rish\n"
        "/majburiy_ochir @guruh — uzish\n/obuna — foydalanuvchini tekshirish",
    "filter": "🚫 *Spam va reklama filtri*\n\n"
        "/filtr — amaldagi filtrlarni ko‘rish\n/filtr + so‘z — filtr qo‘shish\n/filtr - so‘z — filtrni o‘chirish",
    "schedule": "🗓 *Rejalashtirilgan postlar*\n\n"
        "/rejalashtir — mavjud postlarni ko‘rish\n"
        "/rejalashtir 18:30 | E’lon matni — post rejalashtirish",
    "settings": "⚙️ *Sozlamalar*\n\n"
        "/sozlamalar — joriy sozlamalarni ko‘rish\n/til — til imkoniyatlari\n/holat — bot va server holati\n\n"
        "Admin buyruqlari guruhda faqat adminlar uchun ishlaydi.",
}


async def admin_center_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    user = update.effective_user
    if not query or not query.message:
        return
    await query.answer()
    if not private_admin_allowed(user.id if user else None):
        await query.edit_message_text("Bu bo‘lim faqat bot administratorlari uchun.")
        return
    action = query.data.split(":", 1)[1] if query.data and ":" in query.data else "home"
    if action == "close":
        await query.edit_message_text("Admin boshqaruvi yopildi. Qayta ochish uchun /start bosing.")
        return
    if action == "home":
        await query.edit_message_text("⚙️ *Admin boshqaruvi*\n\nKerakli bo‘limni tanlang:", parse_mode="Markdown", reply_markup=admin_control_markup())
        return
    section = ADMIN_CONTROL_SECTIONS.get(action)
    if not section:
        await query.edit_message_text("Admin boshqaruvi: bo‘lim topilmadi.", reply_markup=admin_control_markup())
        return
    await query.edit_message_text(section, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Boshqaruvga qaytish", callback_data="admin_center:home")],
        [InlineKeyboardButton("✖️ Yopish", callback_data="admin_center:close")],
    ]))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    chat = update.effective_chat
    reply_markup = None
    if chat and chat.type == ChatType.PRIVATE and private_admin_allowed(user.id if user else None):
        reply_markup = admin_main_keyboard()
    await update.effective_message.reply_text(
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat.type == ChatType.PRIVATE and private_admin_allowed(user.id):
        if await admin_reply_button(update, text):
            return
    store.ensure_chat(chat.id, chat.title or "")
...
telegram_app.add_handler(CallbackQueryHandler(admin_center_callback, pattern=r"^admin_center:"))
