...
| `BOT_DB_PATH` | SQLite fallback fayli yo‘li; PostgreSQL ishlatilmasa qo‘llanadi |
| `DATABASE_URL` | Ixtiyoriy PostgreSQL ulanish satri; Supabase transaction pooler URI’sini kiriting, parolni GitHub yoki chatga yozmang |
...
Telegram guruhlarida barcha oddiy xabarlarni olish uchun BotFather’dagi Privacy Mode’ni o‘chirish yoki botni administrator qilish kerak. Anti-spam filtri takroriy va reklama xabarlarini javobsiz qoldiradi. SQLite xotirasi processed Telegram update ID’larini saqlaydi. Shu sababli webhook retry yoki server restart paytida ayni update qayta kelib qolsa, bot uni ikkinchi marta javob bermasdan tashlab yuboradi. `DATABASE_URL` sozlanganida shu jadvallar Supabase PostgreSQL’da saqlanadi va Render Free restartlaridan keyin ham qayta tiklanadi; `DATABASE_URL` bo‘lmasa, xavfsiz SQLite fallback ishlaydi. Adminlarning ogohlantirish, jim qilish, bloklash, chiqarish va filtr amallari `moderation_actions` jadvalida qayd etilib, `/statistika` javobida so‘nggi amallar ko‘rsatiladi.

## Ixtiyoriy kengaytmalar

Agar `DATABASE_URL` berilsa, bot Supabase PostgreSQL’ning transaction pooler ulanishidan foydalanadi; bu Render Free kabi tashqi serverlarda IPv4 va qisqa ulanishlar uchun mosroq. Agar ulanish ishlamasa, bot xatoni maxfiy ma’lumotlarsiz loglaydi va noto‘g‘ri bazaga jim yozmaydi. Agar `GROQ_API_KEY` berilsa, bot Groq’ning OpenAI-compatible chat endpoint’i orqali foydalanuvchi tilida haqiqiy AI javoblarini qaytarishga urinadi. `GROQ_API_KEY` ovozli xabarlar uchun ham ishlatiladi; standart model `whisper-large-v3-turbo`. Ovozli xabarlar Groq’ning alohida audio transcription endpoint’iga yuboriladi. Rasmlar xavfsiz hajm limiti bilan qabul qilinadi; Groq vision/OCR mos modeli mavjud bo‘lsa, shu provider orqali ishlaydi. Kalitlar berilmaganida bot fallback rejimida ishlaydi va foydalanuvchiga integratsiya faollashtirilmaganini o‘z tilida tushuntiradi.
